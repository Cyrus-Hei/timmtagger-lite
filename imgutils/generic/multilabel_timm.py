...
class MultiLabelTIMMModel:
    ...
    def _raw_predict_batch(self, images, preprocessor: Literal['test', 'val'] = 'test'):
        ...
        import numpy as np
        input_ = np.stack([trans(img) for img in loaded_images], axis=0)
        output_names = [output.name for output in model.get_outputs()]
        output_values = model.run(output_names, {'input': input_})
    ...
    def predict_batch(self, images, preprocessor: Literal['test', 'val'] = 'test',
        ...
        df_tags = self._open_tags()
        values_batch = self._raw_predict_batch(images, preprocessor=preprocessor)
